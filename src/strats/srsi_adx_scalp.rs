...
impl NeedsIndicators for SrsiAdxScalp{
    fn required_indicators_static() -> Vec<IndexId>{
       vec![
            ...
            (IndicatorKind::SmaOnRsi{periods: 14, smoothing_length: 10}, Hour1,),
            (IndicatorKind::Adx{periods: 10, di_length: 10} , Min15),
        ] 
    }
}
...
impl Strat for SrsiAdxScalp {
    ...
    fn on_tick(
        ...
        let max_size = roundf!((margin * lev as f64) / price, sz_decimals);
        ...
        let order = (|| {
            ...
            if let Some(open) = open_pos{
                if  !self.sl_set{
                    let trigger_px = calc_exit_px(open.side, TriggerKind::Sl, 0.3, open.entry_px, lev);
                    self.sl_set = true;
                    return Some(EngineOrder::new_sl(open.size, trigger_px));
                }
                if rsi_1h_value > 60.0 && (rsi_1h_value - sma_rsi_1h_value < (rsi_1h_value * 0.1)){
                    let limit_px = roundf!(calc_exit_px(open.side, TriggerKind::Tp, 0.003, open.entry_px, lev), px_decimals);
                    let size = roundf!(open.size, sz_decimals);
                    self.closing = true;
                    return Some(EngineOrder::new_limit_close(size, limit_px, None));
                }
            }
            ...
            if open_pos.is_none(){
                let size = roundf!(max_size * 0.95, sz_decimals);
                if size * price < MIN_ORDER_VALUE {
                    return None;
                }
                self.active_window_start = None;
                let limit_px = calc_entry_px(Side::Long , 0.3, price, lev);
